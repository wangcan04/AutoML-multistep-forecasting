...
if __name__ == "__main__":
 ...
 X_trainlist = X_trainlist.reshape(len(X_trainlist),10,1)
 X_testlist = X_testlist.reshape(len(X_testlist),10,1)
 ...
 #count the number of models that can be evaluated within 2 hours
 reg = ak.AutoModel(
    inputs=input_node, outputs=output_node, max_trials=10,tuner="random",overwrite=True
 )
 reg.fit(X_trainlist, y_trainlist)
 end = time()
 print('time cost:', end-start)
 print('count:', 7200/(end-start)*10)
 count = int(7200/(end-start)*10)


 reg = ak.AutoModel(
    inputs=input_node, outputs=output_node, max_trials=count,tuner="random",overwrite=True
 )
 reg.fit(X_trainlist, y_trainlist)
